...
<div x-data="journalApp()" class="min-h-screen relative pb-24">
                ...
                <div @click="openModal('view', index, journal)" 
                    ...
                    <div class="flex justify-between items-start mb-4 relative">
                        ...
                        <div class="absolute right-0 top-0">
                            <button @click.stop="openMenu = !openMenu" @click.away="openMenu = false" class="bg-[#558B2F] hover:bg-[#457225] text-white rounded-lg transition transform hover:scale-105 focus:outline-none shadow-sm w-8 h-8 flex items-center justify-center">
                            ...
                            <div x-show="openMenu" x-cloak class="absolute right-0 top-10 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.15)] rounded-xl py-2 w-32 z-20 border border-gray-100">
                                <button @click.stop="openModal('edit', index, journal); openMenu = false" class="w-full flex items-center px-4 py-2 text-sm text-[#396935] font-semibold hover:bg-green-50 gap-2 text-left">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                    Edit
                                </button>
                                <button @click.stop="deleteJournal(index); openMenu = false" class="w-full flex items-center px-4 py-2 text-sm text-[#396935] font-semibold hover:bg-red-50 gap-2 text-left">
    ...
    <div x-show="isModalOpen" x-cloak class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
    ...
    function journalApp() {
        return {
            ...
            openModal(mode, index = null, data = null) {
                ...
                if (data) {
                    // Copy data, dan pastikan ID terisi (mapping id_jurnal ke id)
                    this.formData = JSON.parse(JSON.stringify(data));
                    this.formData.id = data.id_jurnal || data.id; 
                } else {
                    // Reset form untuk data baru
                    this.formData = { id: null, title: "", content: "", date: "", rating: 0 };
                }
                ...
                .then(result => {
                    if(result.success) {
                        if (this.modalMode === "create") {
                            ...
                            // Update data di tampilan tanpa refresh
            ...
            deleteJournal(index) {
                if(confirm("Hapus cerita ini?")) {
                    // Disini Anda bisa tambahkan fetch delete ke server jika perlu
